...
function generateJS(ast, options) {
  ...
  function generateRuleHeader(ruleNameCode, ruleIndexCode) {
    ...
    if (options.cache) {
      parts.push([
        'var recordedExpects = [];',
        'function cachedFail(description) {',
        '  if (peg$silentFails === 0) {',
        '    peg$fail(description);',
        '  }',
        '  recordedExpects.push(description);',
        '}',
        'var key    = peg$currPos * ' + ast.rules.length + ' + ' + ruleIndexCode + ',',
        '    cached = peg$resultsCache[key];',
        '',
        'if (cached) {',
        '  recordedExpects = cached.recordedExpects;',
        '  if (peg$silentFails === 0) {',
        '    recordedExpects.map(peg$fail);',
        '  }',
  ...
  function generateRuleFooter(ruleNameCode, resultCode) {
    ...
    if (options.cache) {
      parts.push([
        '',
        'peg$resultsCache[key] = { nextPos: peg$currPos, result: ' + resultCode + ', recordedExpects: recordedExpects };'
      ].join('\n'));
    }
